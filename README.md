...
│   └── workflows/
│       └── ci.yml                       ← Step 3: CI/CD pipeline (5 jobs)
│
├── Scripts/                             ← Adım 1 & 2: Otomasyon scriptleri
│   ├── cleanup.sh                       ← Step 2: Automated Appwrite removal
│   ├── install.sh                       ← Step 1: Install script analysis
│   └── verify-cleanup.sh               ← Step 2: Post-removal verification
│
├── config/                              ← Yapılandırma dosyaları
│   └── hadolint.yaml                    ← Dockerfile linter config
│
├── docs/                                ← Step 3–5: Analiz raporları
│   ├── ci-cd-analysis.md               ← Step 3: CI/CD pipeline analizi
│   ├── docker-security-audit.md        ← Step 4: Docker security checklist
│   ├── source-code-analysis.md         ← Step 5: Kaynak kod & JWT analizi
│   └── threat-model.md                 ← Step 5: Full STRIDE threat model
│
├── poc/                                 ← Step 5: Proof of Concept
│   ├── utils/                           ← PoC yardımcı modüller
│   │   ├── __init__.py
│   │   ├── http_client.py              ← HTTP client & header yardımcıları
│   │   └── reporter.py                 ← Rapor üretici
│   ├── idor-demo.py                    ← IDOR/BOLA PoC scripti
│   └── requirements.txt               ← Python bağımlılıkları
│
├── tests/                               ← Unit test süiti
│   ├── __init__.py
│   └── test_idor.py                    ← 27 unit test (4 test sınıfı)
│
├── .dockerignore                        ← Docker imajına dahil edilmeyenler
├── .env.example                         ← Ortam değişkeni şablonu (→ .env)
├── .gitattributes                       ← Satır sonu & diff ayarları
├── .gitignore                           ← Git dışında tutulan dosyalar
├── compose.yaml                         ← Step 4: Docker Compose (3 servis)
├── Dockerfile                           ← Step 4: Multi-stage build
├── LICENSE                              ← MIT License
├── Makefile                             ← Geliştirici kolaylık komutları
├── README.md                            ← Bu dosya
├── SECURITY.md                          ← Güvenlik politikası & etik kullanım
...
```bash
# 1. Repoyu klonla
git clone https://github.com/alituranakt/Appwrite-Security-Analysis-IDOR-BOLA.git
cd Appwrite-Security-Analysis-IDOR-BOLA

# 2. Ortam değişkenlerini hazırla
cp .env.example .env
# .env dosyasını düzenle: APPWRITE_ENDPOINT, proje ID, test hesapları
...
# 7. Docker ile çalıştır (sadece analiz)
docker compose up analysis

# 8. Tam lab ortamı (Appwrite + MariaDB + PoC)
...
> **Automation:** The cleanup procedure below is fully automated in [`Scripts/cleanup.sh`](Scripts/cleanup.sh). Run `Scripts/verify-cleanup.sh` afterwards to confirm zero residual traces with a pass/fail report. Or use: `make cleanup-appwrite`
...
> **This project's CI/CD:** See [`.github/workflows/ci.yml`](.github/workflows/ci.yml) — 5 jobs: lint → test → docs → security → docker. Full analysis in [`docs/ci-cd-analysis.md`](docs/ci-cd-analysis.md).
...
│  DEFENSE MECHANISMS:                                         │
│  ┌────────────────────────────────────────────────────────┐  │
│  │ OK  Permission-based authorization (collection/doc/file)│  │
│  │ OK  Secure-by-default (deny all unless granted)        │  │
│  │ OK  JWT with 15-minute expiration                      │  │
│  │ OK  Webhook signature verification (HMAC-SHA1)         │  │
│  │ OK  Argon2 password hashing                            │  │
│  │ X   No server-side userId field validation             │  │
│  │ X   No project-level user creation restrictions        │  │
│  │ X   No built-in rate limiting on enumeration           │  │
│  └────────────────────────────────────────────────────────┘  │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
